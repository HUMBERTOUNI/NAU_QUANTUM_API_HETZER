...
def download_and_compute(sym, interval, prepost=False):
    ...
    # ═══ SIGNAL INTEGRITY: Only show signals the indicator actually generates ═══
    # No forced persistence — if the indicator doesn't generate a signal with current data,
    # it means the data changed and the signal shouldn't exist.
    # The disk cache ensures data consistency: once downloaded, data stays the same
    # for the cache TTL period, so signals remain stable naturally.
    ...
    # Use actual previous bar close from chart data (more reliable than separate API call)
    prev_close = bars[-2]["close"] if len(bars) >= 2 else get_prev_close(sym)
