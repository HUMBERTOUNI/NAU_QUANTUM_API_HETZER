...
CACHE = {}
CACHE_TTL = 900  # 15 min memory cache
SCAN_CACHE = {}
PREV_CLOSE_CACHE = {}
...
# Signals on CLOSED candles are saved to disk and NEVER disappear
_SIGNAL_DIR = "/tmp/nau_signals"
os.makedirs(_SIGNAL_DIR, exist_ok=True)

def _signal_key(sym, interval):
    return os.path.join(_SIGNAL_DIR, f"{sym}_{interval}.pkl")

def _load_saved_signals(sym, interval):
    """Load previously confirmed signals from disk."""
    try:
        p = _signal_key(sym, interval)
        if not os.path.exists(p): return {}
        with open(p, 'rb') as f: return pickle.load(f)
    except: return {}

def _save_signals(sym, interval, signals_dict):
    """Save confirmed signals to disk. Key = candle timestamp."""
    try:
        with open(_signal_key(sym, interval), 'wb') as f:
            pickle.dump(signals_dict, f)
    except: pass

# ═══ COMPUTED RESULT CACHE ═══
# Cache the FULL computed result (bars + signals + summary) to disk
_RESULT_DIR = "/tmp/nau_results"
os.makedirs(_RESULT_DIR, exist_ok=True)

def _result_cache_get(sym, interval, prepost):
    """Get cached computed result. Returns None if expired."""
    try:
        p = os.path.join(_RESULT_DIR, hashlib.md5(f"{sym}:{interval}:{prepost}".encode()).hexdigest() + ".pkl")
        if not os.path.exists(p): return None
        age = time.time() - os.path.getmtime(p)
        # Long TTL to prevent signal flickering on closed candles
        # 1m: 2 min, 5m: 5 min, 15m+intraday: 15 min, daily+: 30 min
        if interval == "1m": ttl = 120
        elif interval == "5m": ttl = 300
        elif interval in ("15m","30m"): ttl = 900
        elif interval in ("1h","2h","4h"): ttl = 1200
        else: ttl = 1800  # daily, weekly, monthly
        if age > ttl: return None
        with open(p, 'rb') as f: return pickle.load(f)
    except: return None

def _result_cache_set(sym, interval, prepost, data):
    """Save computed result to disk."""
    try:
        p = os.path.join(_RESULT_DIR, hashlib.md5(f"{sym}:{interval}:{prepost}".encode()).hexdigest() + ".pkl")
...
def download_and_compute(sym, interval, prepost=False):
    ...
    # 2. Disk result cache (fast, no recompute)
    disk_cached = _result_cache_get(sym, interval, prepost)
    if disk_cached:
        cache_set(ck, disk_cached)
        return disk_cached
    ...
    # ═══ PERSISTENT SIGNALS: Merge saved signals so closed-candle signals NEVER disappear ═══
    # Load previously saved signals for this symbol+interval
    saved = _load_saved_signals(sym, interval)
    
    # Save NEW signals from closed candles (all except last bar which may still be forming)
    last_bar_time = bars[-1]["time"] if bars else 0
    for sig in signals:
        if sig["time"] != last_bar_time:  # Only save CLOSED candle signals
            saved[sig["time"]] = sig
    
    # Merge: for each saved signal, if not already in current signals, add it
    current_times = {s["time"] for s in signals}
    for t, sig in saved.items():
        if t not in current_times and t != last_bar_time:
            signals.append(sig)
    
    # Sort signals by time
    signals.sort(key=lambda x: x["time"])
    
    # Save the merged signal store to disk
    _save_signals(sym, interval, saved)
    ...
    cache_set(ck, result)
    # Save to disk cache for fast reload
    _result_cache_set(sym, interval, prepost, result)
    return result
...
def scan_one(sym_info, interval, min_conf):
    ...
    try:
        ...
        if conf < min_conf or abs(sig) < 10 or label == "NEUTRAL":
            return None
...
def scan_vc_one(sym_info, interval):
    ...
    try:
        ...
        return {
            "symbol": sym, "name": _get_name(sym), "label": label,
...
def report_worst_performers_4days():
    ...
    for i, p in enumerate(performers[:10]):
        ...
        <td>{_rsi_text(p["rsi"])}</td></tr>"""
    return f"""<div class="rpt-section" style="border-left-color:#ef5350">
<h2>📉 12. Top 10 — PEOR Desempeño Últimos 4 Días</h2>
<table class="rpt-table"><thead><tr><th>#</th><th>Ticker</th><th>Nombre</th><th>Var. 4d</th><th>Precio</th><th>RSI</th></tr></thead>
<tbody>{rows}</tbody></table></div>"""


def report_seguimiento_sb():
    """Section 13: Stocks up and down TODAY — real values from yfinance."""
    candidates = list(dict.fromkeys(SP500[:100] + NASDAQ_100[:50] + DOW_30))
    movers_up, movers_down = [], []
    for sym in candidates[:120]:
        try:
            df, err = safe_download(sym, "5d", "1d", False)
            if err or df is None or df.empty or len(df) < 2: continue
            price = float(df["Close"].iloc[-1])
            prev = float(df["Close"].iloc[-2])
            chg = ((price - prev) / prev) * 100
            entry = {"sym": sym, "name": _get_name(sym), "price": round(price,2), "chg": round(chg,2)}
            if chg > 0.3: movers_up.append(entry)
            elif chg < -0.3: movers_down.append(entry)
        except: pass
    movers_up.sort(key=lambda x: x["chg"], reverse=True)
    movers_down.sort(key=lambda x: x["chg"])
    def rows(items):
        return "".join(f'<tr><td><b>{p["sym"]}</b></td><td>{p["name"]}</td><td>${p["price"]:.2f}</td><td class="{"up" if p["chg"]>0 else "down"}"><b>{p["chg"]:+.2f}%</b></td></tr>' for p in items[:15])
    return f"""<div class="rpt-section" style="border-left-color:#ff9800">
<h2>📊 13. Seguimiento S/B — Acciones que Suben y Bajan Hoy</h2>
<h3 style="color:#26a69a">🟢 Suben hoy ({len(movers_up)})</h3>
<table class="rpt-table"><thead><tr><th>Ticker</th><th>Nombre</th><th>Precio</th><th>Var.</th></tr></thead>
<tbody>{rows(movers_up)}</tbody></table>
<h3 style="color:#ef5350">🔴 Bajan hoy ({len(movers_down)})</h3>
<table class="rpt-table"><thead><tr><th>Ticker</th><th>Nombre</th><th>Precio</th><th>Var.</th></tr></thead>
<tbody>{rows(movers_down)}</tbody></table></div>"""
